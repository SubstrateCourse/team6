...
decl_module! {
	...
	pub struct Module<T: Trait> for enum Call where origin: T::Origin {
		...
		pub fn create_claim(origin, claim: Vec<u8>) -> dispatch::DispatchResult {
			...
			ensure!(claim.len() <= 5, Error::<T>::ClaimTooLong);

			ensure!(!Proofs::<T>::contains_key(&claim), Error::<T>::ProofAleadyExist);
		...
		pub fn transfer_claim(origin, claim: Vec<u8>, receiver: T::AccountId) -> dispatch::DispatchResult {
			...
			Self::deposit_event(RawEvent::ClaimTransferred(sender, claim));
