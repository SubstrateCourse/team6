...
decl_error! {
	pub enum Error for Module<T: Trait> {
		...
		NotClaimOwner,
		ProofTooLong,
		NoteTooLong, // for lesson4 homework
	}
}
...
decl_module! {
	...
	pub struct Module<T: Trait> for enum Call where origin: T::Origin {
		...
		#[weight = 0]
		pub fn create_claim_with_node(origin, claim: Vec<u8>, note: Option<Vec<u8>>) -> dispatch::DispatchResult {
			let sender = ensure_signed(origin)?;
			...
			ProofHash2Detail::<T>::insert(&claim, (sender.clone(), system::Module::<T>::block_number(), <timestamp::Module<T>>::now(), note.clone()));
			Self::deposit_event(RawEvent::ClaimCreated(sender, claim));
